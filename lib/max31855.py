...
class MAX31855(driver.TemperatureDriver):
    ...
    def get_temperature(self, channel: int = 0) -> float:
        ...
        self.read()
        self.check_errors()
        return self.data_to_tc_temperature()
    ...
    def check_errors(self, data_32=None):
        ...
        no_connection = (data_32 & 0x00000001) != 0       # OC bit, D0
        short_to_ground = (data_32 & 0x00000002) != 0      # SCG bit, D1
        short_to_vcc = (data_32 & 0x00000004) != 0         # SCV bit, D2
        if any_errors:
            if no_connection:
                raise driver.DriverError(driver.ERROR_NO_CONNECTION, "Thermocouple not connected.")
            elif short_to_ground:
                raise driver.DriverError(driver.ERROR_SHORT_TO_GROUND, "Thermocouple short to ground.")
            elif short_to_vcc:
                raise driver.DriverError(driver.ERROR_SHORT_TO_VCC, "Thermocouple short to VCC.")
            else:
...
if __name__ == "__main__":
    ...
    for cs_pin in cs_pins:
        thermocouples.append(
            MAX31855({'cs_pin': cs_pin, 'clock_pin': clock_pin, 'data_pin': data_pin, 'pin_mode': 'BCM'})
        )
    running = True
    while running:
        try:
